...
f = function(x, u) {
    ...
    v = x[4:5]; psi = x[6]; tau = exp(psi); 
    #amax = tau;
    amax = 1+log(1+exp(5*(tau-1)))/5
    # tau = 0 -- velocity
    # tau = infi -- acceleration
    return(c(v[1]*cos(x[3]), v[1]*sin(x[3]), v[2], (-v + amax*u)/tau, -t_psi*psi))
}

Fn = function(x,u){
    ## noise in state dynamics
    ## dim = nx X nw , we use nw = nx
    C0 = 0.001*(diag(nw)) #+ 0.01*diag(u) ## (Bu %*% t(Bu))
    #C0[6,6] = 0.01
    C0
}
